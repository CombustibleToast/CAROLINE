...
const getFuncName = (body, regexps) => {
  ...
  for (const regex of regexps) {
    try {
      ...
      if (idx > -1) {
        fn = matchGroup1(`${fn.substring(idx, 0).replaceAll('$', '\\$')}=\\[([a-zA-Z0-9$\\[\\]]{2,})\\]`, body);
      }
    } catch (err) {
      continue;
    }
  }
  if (!fn) throw Error('Could not find signature function name');
  if (fn.includes('[')) throw Error('YouTube changed their response.');
  return fn;
};
...
const extractDecipher = body => {
  try {
    ...
    const funcPattern = `(${decipherFuncName.replaceAll('$', '\\$')}=function\\([a-zA-Z0-9_]+\\)\\{.+?\\})`;
...
const extractNTransform = body => {
  try {
    ...
    const funcPattern = `(${nFuncName.replaceAll('$', '\\$')}=function\\([a-zA-Z0-9_]+\\)\\{.+?.join\\(""\\)});`;
...
exports.decipherFormats = async(formats, html5player, options) => {
  ...
  const decipherScript = functions[0] ? new vm.Script(functions[0]) : null;
  const nTransformScript = functions[1] ? new vm.Script(functions[1]) : null;
