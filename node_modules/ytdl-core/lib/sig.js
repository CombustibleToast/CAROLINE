...
const getFuncName = (body, regexps) => {
  ...
  if (fn.includes('[')) throw Error('YouTube changed their response, please open an issue on ytdl-core GitHub.');
  return fn;
};
...
exports.extractFunctions = body => {
  ...
  const extractDecipher = () => {
    ...
    try {
      const functionPattern = `(${decipherFuncName.replaceAll('$', '\\$')}=function\\([a-zA-Z0-9_]+\\)\\{.+?\\})`;
      const decipherFunction = `var ${matchGroup1(functionPattern, body)};`;
      const helperObjectName = matchGroup1(';([A-Za-z0-9_\\$]{2,})\\.\\w+\\(', decipherFunction).replaceAll('$', '\\$');
  ...
  const extractNTransform = () => {
    // Regex method (slower but more reliable if YouTube changes their response):
    // const N_TRANSFORM_REGEXPS = [
    //   'b=a\\.get\\("n"\\)\\W*?b=([a-zA-Z0-9$\\[\\]]{2,})\\(b\\)',
    // ];
    // const getNTransformFuncName = body => getFuncName(body, N_TRANSFORM_REGEXPS);
    // const nFuncName = getNTransformFuncName(body);
    // if (nFuncName && nFuncName.length) {
    //   const functionPattern = `(${nFuncName}=function\\([a-zA-Z0-9_]+\\)\\{.+?.join\\(""\\)});`;
    //   const nTransformFunction = `var ${matchGroup1(functionPattern, body)};`;
    //   const callerFunction = `${nFuncName}(${N_ARGUMENT});`;
    //   const resultFunction = nTransformFunction + callerFunction;
    //   functions.push(resultFunction);
    // }
