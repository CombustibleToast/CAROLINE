...
module.exports = {
    ...
    async execute(interaction) {
        ...
        //check if the interactor is the gm of the game, if not, reject this interaction
        if(gameData.gmId != interaction.user.id){
            interaction.reply({content: "You are not the GM of this game.", ephemeral: true});
            return;
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`Successfully rejected ${member.user.toString()}.`)
            .setColor(0x5588bb);
        try{
        ...
        try{
            await member.user.send(`Your request to join ${gameData.gameName} has been rejected.`);
        }
        catch(e){
