    ...
    async execute(interaction) {
        ...
        switch (formData.gameType.toLowerCase().charAt(0)) {
            case "c":
                ...
                descriptionString += "Parsed to be a campaign.\n";
                break;
            case "s":
                ...
                descriptionString += "Parsed to be a series.\n";
                break;
            case "o":
                ...
                descriptionString += "Parsed to be a one-shot.\n";
                break;
            default:
                ...
                descriptionString = "Could not determine game type. Please ask the GM and use the /setgametype command.\n";
                break;
        }
        ...
        const newRole = await server.roles.create({
            ...
            .then(() => descriptionString += "Role created successfully.\n");
        await newRole.setPosition(sortingRole.position) //+1 puts it 2 above the sorting role, -1 puts it directly below the sorting role
            .then(() => descriptionString += "Role sorted successfully.\n")
            .catch(e => {
                console.error(`[WARN] Unable to sort the new role for ${formData.gameName}:\n${e}`);
                descriptionString += "Could not sort the role properly. Please move it.\n"
            }); 
        ...
        await interaction.guild.members.fetch(formData.userId)
            .then(member => {
                ...
                descriptionString += "Gave the user the GM role.\n";
            })
            .catch(e => {
                descriptionString += "Was unable to give the user the GM role.\n";
        ...
        const newChannel = await server.channels.create({
            ...
            .then(() => descriptionString += "Channel created.\n")
            .catch(e => {
                console.log(`Unable to create a new channel after accepting a GM form:\n${e}`);
                descriptionString("Unable to create a new channel! (Woe is you if you see this message)\n")
            })
        ...
        const officialReplyEmbed = new EmbedBuilder()
            .setColor(0x00FF00)
            .setTitle("Success")
            .setDescription(`Application approved, channel successfully created.\n${descriptionString}\n\n
